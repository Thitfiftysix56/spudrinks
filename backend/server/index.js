...
app.post(
  process.env.API_PATH + "RegisUser",
...
app.get(process.env.API_PATH + "MannageUser", (req, res) => {
  ...
  db.query(
    `SELECT  @i:=@i+1 AS id,  ID_Person , FName_Person , LName_Person , Username_Person , Password_Person , NameAccount_Person ,CONCAT( DATE_FORMAT( NOW( ) , '%Y' ) +543, '-', DATE_FORMAT( NOW( ) , '%m' ) , '-', DATE_FORMAT( NOW( ) , '%d' ) ) AS Birthday_Person , Phone_Person , Status_Person , Image_Person,alcohol FROM person`,
    (err, result) => {
      if (err) {
...
app.put(
  process.env.API_PATH + "EditUser/:id",
...
app.post(process.env.API_PATH + "Login", (req, res) => {
  ...
  db.query(
    "SELECT * FROM person where Username_Person = ?",
    [userName],
    (err, result) => {
      if (err) {
        res.send({ err: err });
...
app.post(process.env.API_PATH + "qrscans", (req, res) => {
  ...
  const ID_Beverage =
    data === "green margaritaLogin"
      ? "Bev005"
      : data === "Yellow HawaiiLogin"
      ? "Bev001"
      : data === "KamikazeLogin"
      ? "Bev002"
      : data === "Red LadyLogin"
          ...
          db.query(
            ...
            (err, result) => {
              if (err) {
                ...
                return res
                  .status(500)
                  .json({ msg: "Path RegisUser Server error" });
              } else {
                // return res.status(200).json({msg: 'Insert Ok'})
                db.query(
                  "UPDATE bottlevolume SET Volume_Bottle = ? WHERE Id_Bottle = ? ",
                              ...
                              (err, result) => {
                                if (err) {
                                  ...
                                  return res
                                    .status(500)
                                    .json({
                                      msg: "Path RegisUser Server error",
                                    });
                                } else {
                                  // return res.status(200).json({msg: 'Insert Ok'})
                                  db.query(
...
app.get(process.env.API_PATH + "GetSumAlcohol/:ID_Person", (req, res) => {
  ...
  db.query(
    `SELECT * FROM ordering INNER JOIN beverage ON ordering.ID_Beverage = beverage.ID_Beverage where ordering.ID_Person = ? AND DATE(ordering.OrderDate_Order) = ?`,
    [ID_Person, modifiedString],
    (err, result) => {
      if (err) {
        console.log("GetAllData>>>>>> : ", err);
        return res.status(500).json({ msg: "Path GetAllData Server error" });
