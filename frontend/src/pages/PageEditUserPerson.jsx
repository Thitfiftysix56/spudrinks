...
function PageEditUserPerson() {
    ...
    const submit = () => {
        ...
        formData.append('firstName', firstName === '' ? data.FName_Person : firstName)
        formData.append('lastName', lastName === '' ? data.LName_Person : lastName)
        formData.append('userName', userName === '' ? data.Username_Person : userName)
        // formData.append('passWord', passWord)
        formData.append('nameAccount', nameAccount === '' ? data.NameAccount_Person : nameAccount)
        formData.append('birthDay', birthDay === '' ? data.Birthday_Person : birthDay)
        formData.append('numberPhone', numberPhone === '' ? data.Phone_Person : numberPhone)
        // formData.append('status_Person', status_Person)
        formData.append('filename', data.Image_Person)
        formData.append('file',image_Person )
        Axios.put(process.env.REACT_APP_API + `/EditUser/${data.ID_Person}`,formData)
        .then((res) => {
          setAlerts(true)
...
<Box sx={{margin:'10px'}}>
    <Avatar sx={{width:'80px',height:'80px'}}  alt="ไม่มีรูป"   src={imageUrl === null ? require(`../../src/ImgPerson/uploads/${IdUser[0].Image_Person}`) : imageUrl} />
</Box>
<Box sx={{margin:'10px'}}>
<input
                accept="image/*"
...
<CssTextField id="outlined-basic" fullWidth label="Username" variant="outlined" defaultValue={IdUser[0].Username_Person} onChange={e => setUserName(e.target.value)}/>
</Box>

<Box sx={{margin:'10px',width:'100%'}}> 
<CssTextField id="outlined-basic" fullWidth label="ชื่อบัญชีผู้ใช้งาน" variant="outlined" defaultValue={IdUser[0].NameAccount_Person} onChange={e => setNameAccount(e.target.value)}/>
</Box>
<Box sx={{margin:'10px',width:'100%'}}>
<CssTextField id="outlined-basic" fullWidth label="ชื่อ" variant="outlined" defaultValue={IdUser[0].FName_Person} onChange={e => setFirstName(e.target.value)}/>
</Box>
<Box sx={{margin:'10px',width:'100%'}}>
<CssTextField id="outlined-basic" fullWidth label="นามสกุล" variant="outlined" defaultValue={IdUser[0].LName_Person} onChange={e => setLastName(e.target.value)}/>
</Box>
<Box sx={{margin:'10px',width:'100%'}}>
<CssTextField name="someDate" label="เดือน/วัน/ปีเกิด" InputLabelProps={{ shrink: true, required: true }} type="date" fullWidth defaultValue={IdUser[0].Birthday_Person} onChange={e => setBirthDay(e.target.value)}/>
</Box>
<Box sx={{margin:'10px',width:'100%'}}>
<CssTextField id="outlined-basic" fullWidth label="เบอร์โทรศัพท์" variant="outlined" defaultValue={IdUser[0].Phone_Person} onChange={e => setNumberPhone(e.target.value)}/>
</Box>
<Grid container  columns={{ xs: 4, sm: 8, md: 12 }} sx={{margin:'10px'}}>
  ...
  <Box sx={{marginRight:{lg: '10px',
      ...
      sm: '10px',
      xs: '10px'}}}>
  <Link to="/PageSelectBeverage" style={{textDecoration: 'none'}}>
  ...
  <Box sx={{marginLeft:{lg: '10px',
      ...
      sm: '10px',
      xs: '10px'}}}>
   <ColorButton fullWidth variant="contained" sx={{color:'#ffffff'}} onClick={() => submit()}>แก้ไขข้อมูล</ColorButton>
    </Box>
  </Grid>
</Grid>
</Box>
