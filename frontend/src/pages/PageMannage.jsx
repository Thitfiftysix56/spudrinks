...
function PageMannage() {
  ...
  const handleChange = (event, cellValues) => {
    Axios.put(
      process.env.REACT_APP_API +
        `/UpdateStatus/${event.target.value}/${cellValues.row.ID_Person}`
    )
      .then((res) => {
        setAlerts(true);
  ...
  const Delete = (idUser) => {
    Axios.delete(
      process.env.REACT_APP_API +
        `/DeleteUser/${idUser.row.ID_Person}/${idUser.row.Image_Person}`
    )
      .then((res) => {
        setAlerts(true);
  ...
  const columns = [
    ...
    },
    {
      ...
      headerAlign: "center",
      valueGetter: (params) =>
        `${params.row.FName_Person || ""} ${params.row.LName_Person || ""}`,
      align: "center",
    },
    {
